...
public class Code07_CutOrPoison {
    ...
    public static int fast(int[] cuts , int[] posions , int hp){
        ...
        for (int l = 1 , r = hp+1 , m; l <= r ;) {
            ...
            // 传进 刀杀、毒杀、血量、回合数
            // 返回能不能杀死怪兽
            if (f(cuts , posions , hp , m)){
                ans = m ;
                r = m - 1 ; // 如果能杀死，那么回合数再多更能杀死，要求最少回合数，所以左侧二分
            } else {
                l = m + 1 ;
            }
        }
    ...
    // 刀杀、毒杀、血量、回合数
    // 返回在该回合里能不能杀死
    public static boolean f(int[] cuts , int[] posions , long hp , int m){
        int n = Math.min(cuts.length , m) ; // 技能的回合数跟要求的回合数选最小
        for (int i = 0 , j = 1 ; j < n ; i++ , j++){
            // 血量 减去 （刀杀的效果  跟  毒杀的效果（m-j,因为毒杀当前回合不发挥作用，后边一直发挥作用） 取最大伤害）
            hp -= Math.max((long) cuts[i] , (long) (m-j) * (long) posions[i]);
            if (hp<=0){
                return true ;
            }
        }
